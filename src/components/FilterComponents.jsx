...
export function SelectFilter({ icon, value, onChange, options, defaultText }) {
    ...
    return (
        <div className={`relative flex items-center rounded-2xl px-4 py-2.5 text-[13px] shadow-sm border transition-all whitespace-nowrap cursor-pointer flex-1 sm:flex-none min-w-[140px] font-bold group focus-within:ring-2 focus-within:ring-brand-primary/50 ${
            isActive 
...
export function PeriodSelector({ selectedYear, setSelectedYear, selectedMonth, setSelectedMonth }) { 
    ...
    return ( 
        <div className="flex items-center gap-1.5 bg-white dark:bg-darkbg-card p-1.5 rounded-2xl border border-zinc-200/80 dark:border-darkbg-border shadow-sm"> 
            <div className="relative hover:bg-zinc-50 dark:hover:bg-darkbg-hover rounded-xl transition-colors focus-within:ring-2 focus-within:ring-brand-primary/50">
